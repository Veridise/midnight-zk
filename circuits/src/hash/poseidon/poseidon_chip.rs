...
pub mod extraction {
    ...
    impl<F: PrimeField, const QUEUE: usize, C> LoadFromCells<F, C> for LoadedPoseidonState<F, QUEUE> {
        fn load(
            ctx: &mut extractor_support::cells::ctx::ICtx,
            chip: &C,
            layouter: &mut impl midnight_proofs::circuit::Layouter<F>,
            injected_ir: &mut extractor_support::circuit::injected::InjectedIR<F>,
        ) -> Result<Self, extractor_support::error::Error> {
    ...
    impl<F: PrimeField, const QUEUE: usize, C> StoreIntoCells<F, C> for LoadedPoseidonState<F, QUEUE> {
        fn store(
            ...
            layouter: &mut impl midnight_proofs::circuit::Layouter<F>,
            injected_ir: &mut extractor_support::circuit::injected::InjectedIR<F>,
        ) -> Result<(), extractor_support::error::Error> {
